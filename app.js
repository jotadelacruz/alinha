...
let packages = [];
let certificates = [];
let paymentTransactions = []; // histórico real de recebimentos (mês atual)
let clientCredits = []; // saldo a favor de cada cliente
...
async function loadAllData(){
  ...
  const [clientsData, apptsData, paymentsData, billsData, packagesData, transactionsData, creditsData] = await Promise.all([
    db.loadClients(),
    db.loadAppointments(rangeFrom, rangeTo),
    db.loadPayments(currentReferenceMonthISO()),
    db.loadBills(),
    db.loadPackages(),
    db.loadPaymentTransactions(currentReferenceMonthISO()),
    db.loadClientCredits(),
  ]);
...
document.querySelectorAll('.nav-item[data-page]').forEach(btn=>{
  btn.addEventListener('click', ()=>{
    ...
    if(page === 'controle-horario'){
      renderControleHorario();
    }
...
/* ============================================================
   IMPORTAÇÃO DE CLIENTES (planilha CSV)
   ============================================================ */
const IMPORT_TEMPLATE_HEADERS = ['Nome','Telefone','E-mail','Frequência','Dia fixo','Horário fixo','Modalidade','Valor da sessão','Status','Observações'];

function downloadImportTemplate(){
  const exampleRow = ['Maria da Silva','(51) 99999-0000','maria@email.com','Semanal','Segunda','08:00','Presencial','210','Ativo','Cliente exemplo - pode apagar esta linha'];
  const csv = buildCSV(IMPORT_TEMPLATE_HEADERS, [exampleRow]);
  downloadCSV('alinha-modelo-importacao-clientes.csv', csv);
...
function parseCSV(text){
  ...
  text = text.replace(/^\uFEFF/, '');
  const delimiter = text.indexOf(';') !== -1 && text.split('\n')[0].split(';').length >= text.split('\n')[0].split(',').length ? ';' : ',';
...
function openImportClientsModal(){
    ...
    <div class="modal-overlay" id="overlay-import">
      <div class="modal">
        ...
        <div class="modal-body">
          <p class="settings-hint" style="margin-bottom:14px;">Use o modelo de planilha para garantir que as colunas fiquem certas, preencha com seus clientes e envie o arquivo abaixo.</p>
          <button type="button" class="btn btn-ghost" id="btn-download-template" style="margin-bottom:18px;">
          ...
          <div id="import-dropzone" style="border:2px dashed var(--line); border-radius:12px; padding:28px; text-align:center; cursor:pointer; transition:border-color .15s;">
            <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.6" style="color:var(--ink-soft); margin-bottom:10px;"><path d="M21 15v4a2 2 0 01-2 2H5a2 2 0 01-2-2v-4"/><path d="M17 8l-5-5-5 5M12 3v12"/></svg>
            <div style="font-size:13.5px; font-weight:600;">Clique para escolher o arquivo .csv</div>
            <div style="font-size:12px; color:var(--ink-soft); margin-top:4px;">ou arraste o arquivo aqui</div>
          </div>
          <div id="import-preview-area" style="display:none; margin-top:18px;"></div>
          <div class="modal-actions">
            <button type="button" class="btn btn-ghost" id="cancel-import">Cancelar</button>
            <button type="button" class="btn btn-primary" id="confirm-import" style="display:none;">Confirmar importação</button>
          </div>
        </div>
      </div>
    </div>
  ...
  document.getElementById('overlay-import').addEventListener('click', (e)=>{ if(e.target.id==='overlay-import') closeModal(); });
  document.getElementById('btn-download-template').addEventListener('click', downloadImportTemplate);

  const fileInput = document.getElementById('import-clients-file-input');
  const dropzone = document.getElementById('import-dropzone');

  const handleFile = (file) => {
    if(!file) return;
    if(!file.name.toLowerCase().endsWith('.csv')){ showToast('Selecione um arquivo .csv'); return; }
    ...
    reader.onload = (ev) => {
      ...
      if(result.headerError){
        ...
        previewArea.innerHTML = `<div class="records-empty" style="border-color:var(--alert); color:var(--alert);">Não encontramos a coluna "Nome" na planilha. Confira se está usando o modelo correto.</div>`;
        confirmBtn.style.display = 'none';
        return;
      }

      parsedClients = result.valid;
      previewArea.style.display = 'block';

      let html = `<div class="settings-hint" style="margin-bottom:10px;">
        <strong style="color:var(--sage);">${result.valid.length}</strong> ${result.valid.length===1?'cliente pronto':'clientes prontos'} para importar
        ${result.skipped.length > 0 ? ` · <strong style="color:var(--alert);">${result.skipped.length}</strong> ${result.skipped.length===1?'linha foi pulada':'linhas foram puladas'}` : ''}
      </div>`;

      if(result.valid.length > 0){
        html += `<div style="max-height:200px; overflow-y:auto; border:1px solid var(--line); border-radius:10px;">`;
        result.valid.slice(0,50).forEach(c=>{
          html += `<div style="display:flex; justify-content:space-between; padding:8px 12px; border-bottom:1px solid var(--paper-soft); font-size:13px;">
            <span>${escapeHtml(c.name)}</span><span style="color:var(--ink-soft);">${escapeHtml(c.phone || '—')}</span>
          </div>`;
        });
        if(result.valid.length > 50) html += `<div style="padding:8px 12px; font-size:12px; color:var(--ink-soft);">+ ${result.valid.length-50} outros...</div>`;
        html += `</div>`;
      }

      if(result.skipped.length > 0){
        html += `<div style="margin-top:10px; font-size:12px; color:var(--alert);">`;
        result.skipped.slice(0,10).forEach(s=>{ html += `Linha ${s.line}: ${escapeHtml(s.reason)}<br>`; });
        html += `</div>`;
      }

      previewArea.innerHTML = html;
      confirmBtn.style.display = result.valid.length > 0 ? 'inline-flex' : 'none';
    };
    reader.readAsText(file, 'UTF-8');
  };

  dropzone.addEventListener('click', ()=> fileInput.click());
  dropzone.addEventListener('dragover', (e)=>{ e.preventDefault(); dropzone.style.borderColor = 'var(--navy-light)'; });
  ...
  document.getElementById('confirm-import').addEventListener('click', async ()=>{
    ...
    for(const clientData of parsedClients){
      try{
        ...
        console.error('Erro ao importar', clientData.name, err);
        errorCount++;
      }
    }
    ...
    renderClients();
    if(errorCount === 0){
      showToast(`${successCount} ${successCount===1?'cliente importado':'clientes importados'} com sucesso`);
    } else {
      showToast(`${successCount} importados, ${errorCount} com erro`);
    }
  });
}
...
document.getElementById('btn-import-clients').addEventListener('click', openImportClientsModal);
...
/* ============================================================
   ...
   ============================================================ */
/**
 * Calcula a situação financeira de UM cliente num mês de referência:
 * quanto é devido (sessões x valor), quanto já foi recebido (soma de
 * transações daquele mês, incluindo crédito aplicado), e o saldo.
 */
function computeClientFinance(clientId, monthISO){
  const client = clientById(clientId);
  const payment = payments.find(p => p.clientId === clientId); // sessions_count do mês, já carregado por currentReferenceMonthISO()
  const sessions = payment ? payment.sessions : 0;
  const devido = client ? sessions * client.value : 0;

  const transactionsThisMonth = paymentTransactions.filter(t => t.clientId === clientId && t.referenceMonth === monthISO);
  const recebidoDireto = transactionsThisMonth.reduce((s,t) => s + t.amount, 0);

  const credit = clientCredits.find(c => c.clientId === clientId);
  const creditBalance = credit ? credit.balance : 0;
  // Crédito disponível é usado para abater o que falta, até o limite do que é devido
  const restanteAntesDoCredito = Math.max(0, devido - recebidoDireto);
  const creditoAplicado = Math.min(creditBalance, restanteAntesDoCredito);

  const recebidoTotal = recebidoDireto + creditoAplicado;
  const saldo = Math.max(0, devido - recebidoTotal);
  const excedente = Math.max(0, recebidoTotal - devido); // não deveria sobrar, pois creditoAplicado é limitado, mas mantém por segurança

  let status = 'aberto';
  if(devido === 0){ status = recebidoDireto > 0 ? 'pago' : 'aberto'; }
  else if(saldo <= 0){ status = 'pago'; }
  else if(recebidoTotal > 0){ status = 'parcial'; }

  return { client, sessions, devido, recebidoDireto, creditoAplicado, recebidoTotal, saldo, status, transactions: transactionsThisMonth };
}

/** Calcula a situação financeira de TODOS os clientes para o mês de referência atual. */
function computeFinance(){
  const monthISO = currentReferenceMonthISO();
  let totalRecebido = 0, totalAberto = 0, totalSessoes = 0;

  payments.forEach(p=>{
    const fin = computeClientFinance(p.clientId, monthISO);
    if(!fin.client) return;
    totalSessoes += fin.sessions;
    totalRecebido += fin.recebidoTotal;
    totalAberto += fin.saldo;
  });
...
function renderKPIs(){
  ...
  const openCount = payments.filter(p => {
    const fin = computeClientFinance(p.clientId, monthISO);
    return fin.client && fin.saldo > 0;
  }).length;
  const monthLabel = TODAY.toLocaleDateString('pt-BR', {month:'long'});

  document.getElementById('kpi-row').innerHTML = `
    <div class="kpi-card">
      <div class="kpi-label">Recebido em ${monthLabel}</div>
    ...
    <div class="kpi-card">
      ...
      <div class="kpi-trend down">${openCount} ${openCount===1 ? 'cliente pendente' : 'clientes pendentes'}</div>
    </div>
    <div class="kpi-card">
...
function renderOpenList(){
  ...
  const openOnes = payments
    .map(p => computeClientFinance(p.clientId, monthISO))
    .filter(fin => fin.client && fin.saldo > 0);
  ...
  let html = '';
  openOnes.forEach(fin=>{
    html += `<div class="open-item">
      <div>
        <div class="name">${escapeHtml(fin.client.name)}</div>
        <div class="since">${fin.recebidoTotal > 0 ? 'Pagamento parcial' : 'Em aberto'}</div>
      </div>
      <div style="display:flex; align-items:center; gap:10px;">
        <div class="amt">${fmtBRL(fin.saldo)}</div>
        <button class="btn btn-ghost btn-sm" data-register-payment="${fin.client.id}">Registrar</button>
      </div>
    </div>`;
  });
  list.innerHTML = html;
  list.querySelectorAll('[data-register-payment]').forEach(btn=>{
    btn.addEventListener('click', ()=>{
      openPaymentModal(btn.dataset.registerPayment);
    });
  });
}
...
function renderFinTabContent(){
  ...
  if(state.finTab === 'por-cliente'){
    ...
    payments.forEach(p=>{
      const fin = computeClientFinance(p.clientId, monthISO);
      if(!fin.client) return;
      const label = fin.status==='pago' ? 'Pago' : fin.status==='parcial' ? 'Parcial' : 'Em aberto';
      const actionBtns = `
        <button class="btn btn-ghost btn-sm" data-register-payment="${fin.client.id}">Registrar</button>
        ${fin.saldo > 0 ? `<button class="btn btn-ghost btn-sm" data-charge="${fin.client.id}">Cobrar</button>` : ''}
        ${fin.transactions.length > 0 ? `<button class="btn btn-ghost btn-sm" data-view-history="${fin.client.id}">Histórico</button>` : ''}
      `;
      rows += `<tr>
        <td><div class="row-client"><div class="mini-avatar" style="background:${colorFor(fin.client.name)};">${initials(fin.client.name)}</div>${escapeHtml(fin.client.name)}</div></td>
        <td>${fin.sessions} sessões</td>
        <td class="value-mono">${fmtBRL(fin.client.value)}</td>
        <td class="value-mono">${fmtBRL(fin.devido)}</td>
        <td class="value-mono" style="color:var(--sage);">${fmtBRL(fin.recebidoTotal)}</td>
        <td><span class="pay-status ${fin.status}">● ${label}</span></td>
        <td style="display:flex; gap:6px; flex-wrap:wrap;">${actionBtns}</td>
      </tr>`;
    });
    container.innerHTML = `<div class="card" style="overflow:hidden;">
      <table>
        <thead><tr><th>Cliente</th><th>Sessões no mês</th><th>Valor da sessão</th><th>Total devido</th><th>Recebido</th><th>Status</th><th></th></tr></thead>
        <tbody>${rows}</tbody>
      </table>
    </div>`;
    container.querySelectorAll('[data-register-payment]').forEach(btn=>{
    ...
    container.querySelectorAll('[data-view-history]').forEach(btn=>{
      btn.addEventListener('click', ()=> openPaymentHistoryModal(btn.dataset.viewHistory));
    });
  } else if(state.finTab === 'contas-pagar'){
    renderContasPagar(container);
  } else if(state.finTab === 'historico'){
    renderHistoricoGeral(container);
  } else {
...
/** Aba "Histórico de pagamentos": lista todas as transações do mês corrente, mais recentes primeiro. */
function renderHistoricoGeral(container){
  const monthISO = currentReferenceMonthISO();
  const monthTransactions = paymentTransactions
    .filter(t => t.referenceMonth === monthISO)
    .sort((a,b) => b.paymentDate.localeCompare(a.paymentDate));

  if(monthTransactions.length === 0){
    container.innerHTML = `<div class="card" style="padding:40px 20px; text-align:center; color:var(--ink-soft); font-size:13.5px;">
      Nenhum recebimento registrado neste mês ainda.
    </div>`;
    return;
  }
  ...
  monthTransactions.forEach(t=>{
    ...
    if(!client) return;
    const dateLabel = new Date(t.paymentDate+'T12:00:00').toLocaleDateString('pt-BR', {day:'2-digit', month:'2-digit', year:'numeric'});
    rows += `<tr>
      <td><div class="row-client"><div class="mini-avatar" style="background:${colorFor(client.name)};">${initials(client.name)}</div>${escapeHtml(client.name)}</div></td>
      <td>${dateLabel}</td>
      <td>${escapeHtml(t.paymentMethod)}</td>
      <td class="value-mono" style="color:var(--sage);">${fmtBRL(t.amount)}</td>
      <td><button class="btn-danger-text" data-delete-transaction="${t.id}">Excluir</button></td>
    </tr>`;
  });

  container.innerHTML = `<div class="card" style="overflow:hidden;">
    <table>
      <thead><tr><th>Cliente</th><th>Data</th><th>Forma</th><th>Valor</th><th></th></tr></thead>
      <tbody>${rows}</tbody>
    </table>
  </div>`;

  container.querySelectorAll('[data-delete-transaction]').forEach(btn=>{
    btn.addEventListener('click', async ()=>{
      const id = btn.dataset.deleteTransaction;
      btn.disabled = true;
      try{
        await db.deletePaymentTransaction(id);
...
function openPaymentModal(preselectedClientId){
  if(clients.length===0){ showToast('Cadastre um cliente primeiro'); return; }
  const clientOptions = clients.map(c=>`<option value="${c.id}" ${c.id===preselectedClientId?'selected':''}>${escapeHtml(c.name)}</option>`).join('');

  const todayISO = isoDate(TODAY);
  const currentMonthISO = currentReferenceMonthISO();
  // Opções de mês: os 3 meses anteriores + atual, para permitir lançar pendências antigas
  const monthOptions = [];
  for(let i=3; i>=0; i--){
    const d = new Date(TODAY.getFullYear(), TODAY.getMonth()-i, 1);
    const iso = isoDate(d);
    const label = d.toLocaleDateString('pt-BR', {month:'long', year:'numeric'});
    monthOptions.push(`<option value="${iso}" ${iso===currentMonthISO?'selected':''}>${label.charAt(0).toUpperCase()+label.slice(1)}</option>`);
  }
    ...
    <div class="modal-overlay" id="overlay-payment">
      <div class="modal">
        ...
        <form id="form-payment">
          <div class="modal-body">
            <div class="field" style="margin-bottom:14px;"><label>Cliente</label><select id="p-client">${clientOptions}</select></div>
            <div id="p-balance-hint" class="settings-hint" style="margin-bottom:14px;"></div>
            <div class="field-row" style="margin-bottom:14px;">
              <div class="field"><label>Valor recebido (R$)</label><input type="number" min="0.01" step="0.01" id="p-amount" placeholder="210"></div>
              <div class="field"><label>Data do pagamento</label><input type="date" id="p-date" value="${todayISO}"></div>
            </div>
            <div class="field-row" style="margin-bottom:14px;">
              <div class="field"><label>Mês de referência</label><select id="p-month">${monthOptions.join('')}</select></div>
  ...
  const updateBalanceHint = () => {
    ...
    const monthISO = document.getElementById('p-month').value;
    const fin = computeClientFinance(clientId, monthISO);
    const credit = clientCredits.find(c => c.clientId === clientId);
    const creditBalance = credit ? credit.balance : 0;
    let html = `Total devido neste mês: <strong>${fmtBRL(fin.devido)}</strong> · Já recebido: <strong>${fmtBRL(fin.recebidoTotal)}</strong> · Saldo: <strong style="color:${fin.saldo>0?'var(--alert)':'var(--sage)'};">${fmtBRL(fin.saldo)}</strong>`;
    if(creditBalance > 0) html += `<br>Crédito disponível deste cliente: <strong>${fmtBRL(creditBalance)}</strong>`;
    document.getElementById('p-balance-hint').innerHTML = html;
  };
  updateBalanceHint();
  document.getElementById('p-client').addEventListener('change', updateBalanceHint);
  document.getElementById('p-month').addEventListener('change', updateBalanceHint);
  ...
  document.getElementById('form-payment').addEventListener('submit', async (e)=>{
    ...
    const paymentDate = document.getElementById('p-date').value || todayISO;
    const referenceMonthISO = document.getElementById('p-month').value || currentMonthISO;
    ...
    try{
      const created = await db.createPaymentTransaction({ clientId, referenceMonthISO, amount, paymentDate, paymentMethod });
      if(referenceMonthISO === currentMonthISO){
        paymentTransactions.push(created);
      }

      // Recalcula se sobrou excedente para virar crédito (só faz sentido no mês corrente, que é o que temos carregado)
      if(referenceMonthISO === currentMonthISO){
        const fin = computeClientFinance(clientId, currentMonthISO);
        if(fin.devido > 0 && fin.recebidoDireto > fin.devido){
          const novoCredito = fin.recebidoDireto - fin.devido;
          const updatedCredit = await db.setClientCredit(clientId, novoCredito);
          const idx = clientCredits.findIndex(c => c.clientId === clientId);
          if(idx >= 0) clientCredits[idx] = updatedCredit; else clientCredits.push(updatedCredit);
        }
      }
...
/** Modal simples mostrando o extrato de pagamentos de um cliente (todos os meses). */
function openPaymentHistoryModal(clientId){
  const client = clientById(clientId);
  if(!client) return;

  document.getElementById('modal-root').innerHTML = `
    <div class="modal-overlay" id="overlay-history">
      <div class="modal">
        <div class="modal-header">
          <h2>Histórico — ${escapeHtml(client.name.split(' ')[0])}</h2>
          <button class="modal-close" id="close-history-modal">
            <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M18 6L6 18M6 6l12 12"/></svg>
          </button>
        </div>
        <div class="modal-body">
          <div id="history-list" style="display:flex; flex-direction:column; gap:8px; max-height:360px; overflow-y:auto;">
            <div style="text-align:center; padding:20px; color:var(--ink-soft); font-size:13px;">Carregando...</div>
          </div>
          <div class="modal-actions">
            <button type="button" class="btn btn-ghost" id="close-history-2">Fechar</button>
          </div>
        </div>
      </div>
    </div>
  ...
  document.getElementById('close-history-2').addEventListener('click', closeModal);
  document.getElementById('overlay-history').addEventListener('click', (e)=>{ if(e.target.id==='overlay-history') closeModal(); });

  db.loadClientPaymentTransactions(clientId).then(transactions=>{
    const list = document.getElementById('history-list');
    if(!list) return; // modal pode ter sido fechado antes da resposta
    if(transactions.length === 0){
      list.innerHTML = `<div style="text-align:center; padding:20px; color:var(--ink-soft); font-size:13px;">Nenhum recebimento registrado ainda.</div>`;
      return;
    }
    list.innerHTML = transactions.map(t=>{
      const dateLabel = new Date(t.paymentDate+'T12:00:00').toLocaleDateString('pt-BR', {day:'2-digit', month:'2-digit', year:'numeric'});
      const monthLabel = new Date(t.referenceMonth+'T12:00:00').toLocaleDateString('pt-BR', {month:'short', year:'numeric'});
      return `<div style="display:flex; justify-content:space-between; align-items:center; padding:10px 0; border-bottom:1px solid var(--paper-soft);">
        <div>
          <div style="font-size:13.5px; font-weight:600;">${dateLabel} · ${escapeHtml(t.paymentMethod)}</div>
          <div style="font-size:12px; color:var(--ink-soft);">Referente a ${monthLabel}</div>
        </div>
        <div class="value-mono" style="color:var(--sage); font-weight:600;">${fmtBRL(t.amount)}</div>
      </div>`;
    }).join('');
  }).catch(err=>{
    console.error(err);
    const list = document.getElementById('history-list');
    if(list) list.innerHTML = `<div style="text-align:center; padding:20px; color:var(--alert); font-size:13px;">Erro ao carregar histórico.</div>`;
  });
}
...
function openChargeModal(clientId){
  const client = clientById(clientId);
  if(!client) return;
  const fin = computeClientFinance(clientId, currentReferenceMonthISO());
  if(fin.saldo <= 0){ showToast('Este cliente não tem saldo em aberto'); return; }

  const message = buildChargeMessage(client, fin);
...
function renderResumoDiario(){
  ...
  document.getElementById('resumo-date-label').textContent = dateLabel.charAt(0).toUpperCase() + dateLabel.slice(1);

  const totalClientes = todayAppts.length;
  const primeiraHora = totalClientes > 0 ? todayAppts[0].time : '—';
  const ultimaHora = totalClientes > 0 ? todayAppts[todayAppts.length-1].time : '—';

  const faturamentoDia = todayAppts.reduce((sum, a) => {
    const client = clientById(a.clientId);
    return sum + (client ? client.value : 0);
  }, 0);

  document.getElementById('resumo-kpi-row').innerHTML = `
    <div class="kpi-card">
      <div class="kpi-label">Clientes hoje</div>
      <div class="kpi-value">${totalClientes}</div>
      <div class="kpi-trend">${totalClientes===1 ? 'consulta agendada' : 'consultas agendadas'}</div>
    </div>
    <div class="kpi-card">
      <div class="kpi-label">Faturamento do dia</div>
      <div class="kpi-value mono">${fmtBRL(faturamentoDia)}</div>
      <div class="kpi-trend">valor agendado</div>
    </div>
    <div class="kpi-card">
      <div class="kpi-label">Primeira consulta</div>
      <div class="kpi-value mono">${primeiraHora}</div>
    </div>
    <div class="kpi-card">
      <div class="kpi-label">Última consulta</div>
      <div class="kpi-value mono">${ultimaHora}</div>
    </div>
  `;

  const list = document.getElementById('resumo-clients-list');
  if(todayAppts.length === 0){
    list.innerHTML = `<div class="empty-day">Nenhuma consulta agendada para hoje.</div>`;
    return;
  }
  ...
  todayAppts.forEach(a=>{
    ...
    const tagLabel = a.modality==='Online' ? 'Online' : (a.status==='pending' ? 'A confirmar' : 'Confirmada');
    html += `
      <div class="resumo-client-row">
        <span class="resumo-time">${a.time}</span>
        <div class="client-avatar" style="width:34px; height:34px; font-size:13px; background:${colorFor(client.name)};">${initials(client.name)}</div>
        <div style="flex:1;">
          <div style="font-weight:600; font-size:13.5px;">${escapeHtml(client.name)}</div>
          <div style="font-size:12px; color:var(--ink-soft);">${escapeHtml(client.frequency)} · ${fmtBRL(client.value)}</div>
        </div>
        <span class="tag ${tagCls}">${tagLabel}</span>
      </div>
    `;
...
let activeTimerState = null; // { apptId, clientId, startedAt(ms), durationMin }

function getTodayAppointmentsSorted(){
  const todayISO = isoDate(TODAY);
  return appointments
    .filter(a => a.dateISO === todayISO)
    .sort((a,b) => a.time.localeCompare(b.time));
...
function renderControleHorario(){
  updateNotificationBanner();
  ...
  const todayAppts = getTodayAppointmentsSorted();

  if(todayAppts.length === 0){
    content.innerHTML = `<div class="timer-empty">
      <h3>Nenhuma consulta hoje</h3>
      <p>Quando houver consultas agendadas para hoje, elas aparecerão aqui para você iniciar o controle de horário.</p>
    </div>`;
    return;
  }

  let html = `<div class="timer-idle-list">`;
  todayAppts.forEach(a=>{
    const client = clientById(a.clientId);
    if(!client) return;
    html += `
      <div class="timer-idle-item" data-start-appt="${a.id}">
        <div style="display:flex; align-items:center; gap:12px;">
          <span class="mono" style="font-size:13px; color:var(--ink-soft); width:46px;">${a.time}</span>
          <div class="client-avatar" style="width:34px; height:34px; font-size:13px; background:${colorFor(client.name)};">${initials(client.name)}</div>
          <span style="font-weight:600; font-size:13.5px;">${escapeHtml(client.name)}</span>
        </div>
        <button class="btn btn-primary btn-sm">Iniciar consulta</button>
      </div>
    `;
  });
  html += `</div>`;
  ...
  content.querySelectorAll('[data-start-appt]').forEach(el=>{
    el.addEventListener('click', ()=> startTimer(el.dataset.startAppt));
  });
}

function startTimer(apptId){
  ...
  activeTimerState = {
    ...
    startedAt: Date.now(),
    durationMin: appSettings.agenda.sessionDuration || 50,
    warned: false,
  };
...
function renderActiveTimerView(){
  const content = document.getElementById('timer-content');
  const client = clientById(activeTimerState.clientId);
  if(!client){ stopTimer(); return; }

  const totalSeconds = activeTimerState.durationMin * 60;

  const nextAppt = getTodayAppointmentsSorted().find(a => a.time > timeFromAppt(activeTimerState.apptId) && a.id !== activeTimerState.apptId);
  const nextClient = nextAppt ? clientById(nextAppt.clientId) : null;

  content.innerHTML = `
    <div class="timer-card" id="timer-card">
      <div class="timer-client-name">${escapeHtml(client.name)}</div>
      <div class="timer-client-meta">${escapeHtml(client.frequency)} · ${client.modality}</div>
      <div class="timer-display" id="timer-display">--:--</div>
      <div class="timer-status-label" id="timer-status-label">Sessão em andamento</div>
      <div class="timer-actions">
        <button class="btn btn-ghost" id="btn-stop-timer">Encerrar sessão</button>
      </div>
    </div>
    ${nextClient ? `
    <div class="timer-next-up">
      <div>
        <div class="label">Próxima consulta</div>
        <div style="font-weight:600; font-size:13.5px;">${escapeHtml(nextClient.name)} · ${nextAppt.time}</div>
      </div>
      <div class="client-avatar" style="width:30px; height:30px; font-size:12px; background:${colorFor(nextClient.name)};">${initials(nextClient.name)}</div>
    </div>` : ''}
  `;
  ...
  tickTimer(totalSeconds);
  activeTimerInterval = setInterval(()=> tickTimer(totalSeconds), 1000);
}

function timeFromAppt(apptId){
  const appt = appointments.find(a => a.id === apptId);
  return appt ? appt.time : '';
}

function tickTimer(totalSeconds){
  if(!activeTimerState) return;
  const display = document.getElementById('timer-display');
  const card = document.getElementById('timer-card');
  const statusLabel = document.getElementById('timer-status-label');
  if(!display || !card) return; // usuário pode ter navegado pra outra página

  const elapsedSeconds = Math.floor((Date.now() - activeTimerState.startedAt) / 1000);
  const remaining = totalSeconds - elapsedSeconds;

  const absRemaining = Math.abs(remaining);
  const mm = String(Math.floor(absRemaining / 60)).padStart(2,'0');
  const ss = String(absRemaining % 60).padStart(2,'0');
  display.textContent = (remaining < 0 ? '+' : '') + mm + ':' + ss;

  card.classList.remove('warning','overtime');
  if(remaining <= 0){
    card.classList.add('overtime');
    statusLabel.textContent = 'Tempo excedido';
  } else if(remaining <= 5 * 60){
    card.classList.add('warning');
    statusLabel.textContent = 'Faltam poucos minutos';
  } else {
    statusLabel.textContent = 'Sessão em andamento';
  }

  // Dispara o aviso de "faltam 5 minutos" uma única vez
  if(remaining <= 5*60 && remaining > 5*60 - 2 && !activeTimerState.warned){
    activeTimerState.warned = true;
    const client = clientById(activeTimerState.clientId);
    playTimerSound();
    sendTimerNotification('Faltam 5 minutos', client ? `A sessão com ${client.name} está terminando.` : 'A sessão está terminando.');
  }
}
...
function playTimerSound(){
  ...
  } catch(e){ /* navegador pode bloquear áudio sem interação prévia; ignora silenciosamente */ }
}

function sendTimerNotification(title, body){
  if(!('Notification' in window)) return;
  if(Notification.permission === 'granted'){
    try{ new Notification(title, { body, icon: profile.photoDataUrl || undefined }); }
    catch(e){ console.error(e); }
  }
}

function updateNotificationBanner(){
  const banner = document.getElementById('timer-permission-banner');
  if(!banner) return;
...
function exportBackupJSON(){
  const payload = {
    ...
    profile, appSettings,
    clients, appointments, payments, bills, sessionRecords, packages, certificates, paymentTransactions, clientCredits,
  };
